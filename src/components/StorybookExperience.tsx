...
interface ChapterData {
  ...
  captionEn: string;
  captionFr: string;
  vignetteType: 'cottage' | 'forest' | 'crossroads' | 'feast';
}

const OPENING_CHAPTERS: ChapterData[] = [
  {
    id: 0,
    titleEn: 'Before the Morning Came',
    titleFr: 'Avant que ne Vienne le Matin',
    subtitleEn: 'Chapter I • The Solitary Hearth',
    subtitleFr: 'Chapitre I • Le Foyer Solitaire',
    dropCapEn: 'O',
    dropCapFr: 'I',
    paragraph1En: 'nce upon a time, before the mornings learned to warm the earth, a solitary Witch set forth upon the road.',
    paragraph1Fr: 'l était une fois, avant que les matins n’apprennent à réchauffer la terre, une sorcière solitaire s’avança sur le chemin.',
    paragraph2En: 'In her hands, she carried the blazing heart of the Sun—heavy, brilliant, and waiting to be shared with those who walked in shadow.',
    paragraph2Fr: 'Dans ses mains, elle portait le cœur étincelant du Soleil—lourd, éclatant, et prêt à être partagé avec ceux qui erraient dans l’ombre.',
    quoteEn: '“A single spark is enough to guide a soul through the coldest midnight.”',
    quoteFr: '« Une seule étincelle suffit pour guider une âme à travers la nuit la plus glacée. »',
    captionEn: 'The twilight cottage before the journey begins…',
    captionFr: 'La chaumière au crépuscule avant le grand départ…',
    vignetteType: 'cottage',
  },
  {
    id: 1,
    titleEn: 'The Whispering Road',
    titleFr: 'Le Chemin des Chuchotements',
    subtitleEn: 'Chapter II • Footprints in the Frost',
    subtitleFr: 'Chapitre II • Empreintes dans le Givre',
    dropCapEn: 'T',
    dropCapFr: 'À',
    paragraph1En: 'hrough silent pines and crystalline mist, gentle woodland companions gathered along the winding trails to catch a glimpse of the golden lantern.',
    paragraph1Fr: ' travers les pins silencieux et la brume de cristal, de doux compagnons des bois se rassemblèrent le long des sentiers pour apercevoir la lanterne dorée.',
    paragraph2En: 'The little marmot peaked from behind frosted stones, comforted by the gentle light that melted the mountain chill.',
    paragraph2Fr: 'La petite marmotte émergea de derrière les pierres givrées, réconfortée par la douce lumière qui dissipait la fraîcheur montagnarde.',
    quoteEn: '“Do not fear the dark road ahead; kindness leaves a trail of stars.”',
    quoteFr: '« Ne crains pas la route obscure ; la bienveillance sème une traînée d’étoiles. »',
    captionEn: 'Starlight glowing through the Whispering Pines…',
    captionFr: 'La lueur des étoiles à travers les Pins Murmurants…',
    vignetteType: 'forest',
  },
  {
    id: 2,
    titleEn: 'The Gathering Light',
    titleFr: 'La Lumière Rassemblée',
    subtitleEn: 'Chapter III • The Crossroads of Glass',
    subtitleFr: 'Chapitre III • La Croisée des Verres',
    dropCapEn: 'E',
    dropCapFr: 'C',
    paragraph1En: 'very spark given away along the lonely path did not diminish the Sun—it only made its radiance warmer, purer, and boundless.',
    paragraph1Fr: 'haque étincelle offerte le long du chemin solitaire n’affaiblit pas le Soleil—elle rendit seulement son éclat plus doux, plus pur et infini.',
    paragraph2En: 'For in the crucible of giving, Wendy’s spirit forged friendships stronger than the deepest abyss and brighter than the dawn.',
    paragraph2Fr: 'Car dans le creuset du partage, l’esprit de Wendy forgea des amitiés plus fortes que l’abîme et plus radieuses que l’aurore.',
    quoteEn: '“Now step into the chronicle, and let your warmth awaken the world.”',
    quoteFr: '« Entre maintenant dans la légende, et laisse ta chaleur éveiller le monde. »',
    captionEn: 'The crossroads where every path leads toward dawn…',
    captionFr: 'La croisée des chemins où chaque pas mène vers l’aube…',
    vignetteType: 'crossroads',
  },
];
...
export const StorybookExperience: React.FC<StorybookExperienceProps> = ({
  ...
  const [animationState, setAnimationState] = useState<'idle' | 'opening' | 'turning' | 'closing' | 'page_flip'>('idle');
  const [magicInkKey, setMagicInkKey] = useState<number>(1);
  const [stardustTrails, setStardustTrails] = useState<Array<{ id: number; x: number; y: number; size: number }>>([]);
  ...
  // Trigger magic ink reveal & sound effect whenever spread opens or chapter changes
  const triggerMagicInkReveal = () => {
    setMagicInkKey((prev) => prev + 1);
    audioSynth.playMagicInkWriting();
  };
  ...
  const handleOpenBook = () => {
    ...
    setTimeout(() => {
      ...
      triggerMagicInkReveal();
    }, 950);
  };

  const handleTurnPage = (direction: 'next' | 'prev') => {
    audioSynth.playSoundEffect('page_turn');
    setAnimationState('page_flip');

    setTimeout(() => {
      if (direction === 'next') {
        setCurrentChapter((prev) => Math.min(prev + 1, OPENING_CHAPTERS.length - 1));
      } else {
  ...
  // Interactive Parchment Magic Ink Hover Stardust Trail
  const handleParchmentMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (Math.random() > 0.4) {
      ...
      setStardustTrails((prev) => [...prev.slice(-18), newParticle]);
    }
  };

  const activeChapter = OPENING_CHAPTERS[currentChapter] || OPENING_CHAPTERS[0];

  return (
    <div
      className="fixed inset-0 z-50 flex flex-col items-center justify-between bg-slate-950/95 backdrop-blur-md p-4 sm:p-6 select-none animate-fade-in overflow-y-auto"
      onMouseMove={handleParchmentMouseMove}
    >
      {/* Background Floating Golden Stardust */}
      ...
      <header className="w-full max-w-5xl flex items-center justify-between z-30 mb-2">
        <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-900/80 backdrop-blur-md border border-amber-500/30 text-amber-200 text-xs font-serif shadow-lg">
          ...
          <span>
            {language === 'en'
              ? 'The Illustrated Chronicle • Magic Ink Edition'
              : 'Le Grimoire Illustré • Édition Encre d’Or'}
          </span>
        </div>

        <div className="flex items-center gap-3">
          {/* Re-ink Page / Magic Ink Reveal Button */}
          {step === 1 && (
            <button
              id="storybook-btn-reink"
              onClick={triggerMagicInkReveal}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/40 text-amber-300 hover:text-amber-100 text-xs font-serif transition-all shadow-md cursor-pointer group"
              title={language === 'en' ? 'Relight text with Magic Ink' : 'Révéler à l’Encre Magique'}
            >
              <PenTool className="w-3.5 h-3.5 text-amber-400 group-hover:rotate-12 transition-transform" />
              <span className="hidden sm:inline">
                {language === 'en' ? 'Magic Ink ✨' : 'Encre d’Or ✨'}
              </span>
            </button>
          )}
      ...
      <div
        ref={bookRef}
        className="my-auto w-full flex items-center justify-center p-2 sm:p-4 perspective-storybook relative"
      >
        {/* Interactive Floating Stardust Motes on Hover */}
        {stardustTrails.map((particle) => (
          <div
            key={particle.id}
            className="absolute rounded-full bg-gradient-to-r from-amber-300 via-yellow-200 to-amber-400 pointer-events-none animate-golden-sparkle z-40"
            style={{
              left: `${particle.x}px`,
              top: `${particle.y}px`,
              width: `${particle.size}px`,
              height: `${particle.size}px`,
              boxShadow: '0 0 10px rgba(251, 191, 36, 0.9), 0 0 20px rgba(245, 158, 11, 0.6)',
            }}
          />
        ))}
        ...
        {mode === 'opening' && step === 1 && (
          <div className="relative group w-full max-w-4xl">
            ...
            <div
              className={`relative w-full bg-[#fdfbf6] text-slate-900 rounded-2xl shadow-[0_25px_80px_rgba(0,0,0,0.92),0_0_50px_rgba(245,158,11,0.25)] border-4 border-amber-800/80 p-6 sm:p-10 flex flex-col md:flex-row gap-6 sm:gap-10 transition-all duration-700 preserve-3d ${
            ...
            >
              ...
              {/* LEFT PAGE: Illustrated Vignette with Dynamic Scenery */}
              <div className="flex-1 flex flex-col items-center justify-between p-4 border border-amber-900/20 rounded-xl bg-gradient-to-b from-amber-50/90 to-amber-100/60 text-center shadow-sm">
                {/* Chapter Index Badge */}
                <div className="w-full flex items-center justify-between px-2 text-[11px] font-serif uppercase tracking-widest text-amber-800/80 font-semibold mb-2">
                  <span>{language === 'en' ? `Page ${currentChapter + 1} of 3` : `Page ${currentChapter + 1} sur 3`}</span>
                  <span className="flex items-center gap-1">
                    <Feather className="w-3 h-3 text-amber-600" />
                    <span>{language === 'en' ? 'Magic Ink' : 'Encre d’Or'}</span>
                  </span>
                </div>

                {/* Ornate Dynamic Illustration Frame */}
                <div className="relative w-48 h-48 sm:w-56 sm:h-56 rounded-full bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-900 p-2 shadow-inner border-2 border-amber-700/70 flex items-center justify-center overflow-hidden mb-3">
                  {activeChapter.vignetteType === 'cottage' && (
                  ...
                  {activeChapter.vignetteType === 'forest' && (
                    <svg viewBox="0 0 200 200" className="w-full h-full">
                      {/* Deep Pine Forest Twilight */}
                      <circle cx="50" cy="35" r="1.5" fill="#fde047" className="animate-pulse" />
                      <circle cx="150" cy="25" r="2" fill="#ffffff" className="animate-pulse" />
                      {/* Pines */}
                      <polygon points="40,160 60,90 80,160" fill="#064e3b" />
                      <polygon points="120,160 140,80 160,160" fill="#065f46" />
                      <polygon points="80,170 100,70 120,170" fill="#022c22" />
                      {/* Cute Marmot Silhouette on rock */}
                      <ellipse cx="100" cy="155" rx="14" ry="12" fill="#78350f" />
                      <circle cx="100" cy="142" r="8" fill="#78350f" />
                      <circle cx="100" cy="148" r="4" fill="#fbbf24" className="animate-pulse" />
                      <ellipse cx="100" cy="175" rx="45" ry="16" fill="#1e293b" />
                    </svg>
                  )}

                  {activeChapter.vignetteType === 'crossroads' && (
                    <svg viewBox="0 0 200 200" className="w-full h-full">
                      {/* Golden Dawn Break */}
                      <circle cx="100" cy="70" r="30" fill="#f59e0b" opacity="0.8" className="animate-pulse" />
                      <circle cx="100" cy="70" r="18" fill="#fef08a" />
                      {/* Kiln Hearth */}
                      <rect x="70" y="125" width="60" height="45" rx="8" fill="#78350f" />
                      <ellipse cx="100" cy="145" rx="18" ry="14" fill="#ea580c" className="animate-pulse" />
                      <circle cx="100" cy="145" r="8" fill="#fef08a" />
                      {/* Crossroads stones */}
                      <ellipse cx="100" cy="185" rx="90" ry="25" fill="#292524" />
                    </svg>
                  )}
                </div>

                <span className="text-xs font-serif italic text-amber-900/85 font-medium px-2">
                  {language === 'en' ? activeChapter.captionEn : activeChapter.captionFr}
                </span>

                {/* Chapter Flip Pagination Controls */}
                <div className="flex items-center gap-2 mt-3">
                  <button
                    id="storybook-btn-ch-prev"
                    disabled={currentChapter === 0}
                    onClick={() => handleTurnPage('prev')}
                    className={`p-1.5 rounded-full border border-amber-700/30 text-amber-900 transition-colors ${
                      currentChapter === 0 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-amber-200/60 cursor-pointer'
                    }`}
                    ...
                    {OPENING_CHAPTERS.map((ch, idx) => (
                      <button
                        ...
                        className={`w-2.5 h-2.5 rounded-full transition-all cursor-pointer ${
                          idx === currentChapter
                            ? 'bg-amber-600 w-5 shadow-sm'
                            : 'bg-amber-900/30 hover:bg-amber-800/60'
                        }`}
                        title={language === 'en' ? `Go to Page ${idx + 1}` : `Aller à la page ${idx + 1}`}
                      />
                    ))}
                  </div>

                  <button
                    id="storybook-btn-ch-next"
                    disabled={currentChapter === OPENING_CHAPTERS.length - 1}
                    onClick={() => handleTurnPage('next')}
                    className={`p-1.5 rounded-full border border-amber-700/30 text-amber-900 transition-colors ${
                      currentChapter === OPENING_CHAPTERS.length - 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-amber-200/60 cursor-pointer'
                    }`}
              ...
              {/* RIGHT PAGE: Story Text with Golden Magic Ink Reveal */}
              <div key={`page-${currentChapter}-${magicInkKey}`} className="flex-1 flex flex-col justify-between p-2">
                <div>
                  <div className="flex items-center gap-2 text-amber-800 text-xs font-serif font-bold uppercase tracking-widest mb-2 animate-magic-ink-reveal">
                    <Sparkles className="w-3.5 h-3.5 text-amber-600 animate-pulse" />
                    <span>{language === 'en' ? activeChapter.subtitleEn : activeChapter.subtitleFr}</span>
                  </div>

                  <h2 className="text-2xl sm:text-3xl font-serif font-bold text-amber-950 mb-4 leading-snug animate-magic-ink-heading">
                    {language === 'en' ? activeChapter.titleEn : activeChapter.titleFr}
                  </h2>

                  {/* Paragraph 1 with Illuminated Drop Cap */}
                  <div className="font-serif text-sm sm:text-base text-stone-800 leading-relaxed mb-4 flex items-start">
                    <span className="text-3xl sm:text-4xl font-serif font-bold text-amber-700 leading-none mr-2 mt-0.5 px-2 py-0.5 rounded bg-amber-100/70 border border-amber-600/40 shadow-sm animate-drop-cap">
                      {language === 'en' ? activeChapter.dropCapEn : activeChapter.dropCapFr}
                    </span>
                    <span className="animate-magic-ink-reveal">
                      {language === 'en' ? activeChapter.paragraph1En : activeChapter.paragraph1Fr}
                    </span>
                  </div>

                  {/* Paragraph 2 */}
                  <p className="font-serif text-sm sm:text-base text-stone-800 leading-relaxed mb-4 animate-magic-ink-reveal" style={{ animationDelay: '0.2s' }}>
                    {language === 'en' ? activeChapter.paragraph2En : activeChapter.paragraph2Fr}
                  </p>

                  {/* Illuminated Quote / Moral */}
                  {activeChapter.quoteEn && (
                    <p className="font-serif text-sm sm:text-base text-stone-800 leading-relaxed italic border-l-2 border-amber-600 pl-3 bg-amber-500/5 py-1.5 rounded-r animate-magic-ink-reveal" style={{ animationDelay: '0.4s' }}>
                      {language === 'en' ? activeChapter.quoteEn : activeChapter.quoteFr}
                    </p>
                  )}
                </div>

                {/* Bottom Actions */}
                <div className="mt-6 flex items-center justify-between pt-4 border-t border-amber-900/10">
                  <button
                  ...
                  <div className="flex items-center gap-2">
                    {currentChapter < OPENING_CHAPTERS.length - 1 ? (
                      <button
                        id="storybook-btn-next-chapter"
                        onClick={() => handleTurnPage('next')}
                        className="group px-5 py-2.5 rounded-xl bg-amber-700 hover:bg-amber-600 text-white font-serif font-semibold text-xs sm:text-sm shadow-md transition-all flex items-center gap-1.5 cursor-pointer border border-amber-500/40"
                      >
                        <span>{language === 'en' ? 'Turn Page 📖' : 'Page Suivante 📖'}</span>
        ...
        {mode === 'ending' && step === 1 && (
          <div className="relative group w-full max-w-4xl">
            ...
            >
              ...
              <div className="flex-1 flex flex-col items-center justify-between p-4 border border-amber-900/20 rounded-xl bg-gradient-to-b from-amber-50/90 to-amber-100/60 text-center shadow-sm">
                <div className="w-full flex items-center justify-between px-2 text-[11px] font-serif uppercase tracking-widest text-amber-800/80 font-semibold mb-2">
                  <span>{language === 'en' ? 'Epilogue' : 'Épilogue'}</span>
                  <span className="flex items-center gap-1">
                    <Feather className="w-3 h-3 text-amber-600" />
                    <span>{language === 'en' ? 'Magic Ink' : 'Encre d’Or'}</span>
                  </span>
                ...
                <div className="relative w-48 h-48 sm:w-56 sm:h-56 rounded-full bg-gradient-to-tr from-amber-400 via-yellow-200 to-amber-500 p-2 shadow-inner border-2 border-amber-600 flex items-center justify-center overflow-hidden mb-3">
                  <svg viewBox="0 0 200 200" className="w-full h-full">
                    {/* Radiant Morning Sky */}
                ...
                <div className="flex items-center gap-1.5 text-xs text-amber-700/80 font-serif mt-2">
                  <Sparkles className="w-3.5 h-3.5 text-amber-600" />
                  <span>{language === 'en' ? 'The Sun Returned to Heaven' : 'Le Soleil Retrouvé'}</span>
                </div>
              </div>

              {/* RIGHT PAGE: Epilogue Narration with Golden Magic Ink Reveal */}
              <div key={`epilogue-${magicInkKey}`} className="flex-1 flex flex-col justify-between p-2">
                <div>
                  <div className="flex items-center gap-2 text-amber-800 text-xs font-serif font-bold uppercase tracking-widest mb-3 animate-magic-ink-reveal">
                    <Heart className="w-3.5 h-3.5 text-rose-500 fill-current" />
                    <span>{language === 'en' ? 'The Final Chapter' : 'Le Dernier Chapitre'}</span>
                  </div>

                  <h2 className="text-2xl sm:text-3xl font-serif font-bold text-amber-950 mb-3 leading-snug animate-magic-ink-heading">
                    {language === 'en'
                      ? 'The Light Returned'
                      : 'La Lumière Retrouvée'}
                  </h2>

                  <div className="font-serif text-sm sm:text-base text-stone-900 leading-relaxed font-medium mb-3 flex items-start">
                    <span className="text-3xl sm:text-4xl font-serif font-bold text-amber-700 leading-none mr-2 mt-0.5 px-2 py-0.5 rounded bg-amber-100/70 border border-amber-600/40 shadow-sm animate-drop-cap">
                      {language === 'en' ? 'A' : 'E'}
                    </span>
                    <span className="animate-magic-ink-reveal">
                  ...
                  {/* Mélo Clown Personal Dedication with Golden Pen Styling */}
                  <div className="p-3.5 bg-amber-500/10 border border-amber-700/30 rounded-xl mb-3 shadow-inner animate-magic-ink-reveal" style={{ animationDelay: '0.3s' }}>
                  ...
                  <p className="font-serif text-xs sm:text-sm text-stone-700 italic animate-magic-ink-reveal" style={{ animationDelay: '0.5s' }}>
                    {language === 'en'
      ...
      {/* Footer hint */}
      <footer className="text-center text-amber-200/50 text-xs font-serif mt-2 z-30 flex items-center justify-center gap-2">
        <Sparkles className="w-3.5 h-3.5 text-amber-400/70 animate-pulse" />
        <span>
          {language === 'en'
            ? 'Hover across the parchment to weave golden stardust • Turn pages to reveal the tale'
            : 'Survolez le parchemin pour semer de la poussière d’étoiles • Tournez les pages pour révéler le conte'}
        </span>
      </footer>
    </div>
  );
